...
export namespace Utils {
   ...
   * ```
   */
  function each<T = object> (obj: T, iterator: (value: any, key?: string, obj?: T) => void, context: object): void;
  function each<U = any, T = U[]> (obj: T, iterator: (value: U, key?: number, obj?: T) => void, context: object): void;
   ...
   * @param { boolean } notCover 是否不要覆盖源对象已有的属性
   */
  function deepExtend<T = object, U = object> (target: T, sources: U, notCover?: boolean): T & U;
   ...
   * ```
   */
  function paralle <T = number | object | any[]> (v1: T, v2: T, op: (v1: T, v2: T) => T): T;


  type parallelizeCallback<T = number | object | any[]> = (v1: T, v2: T) => T;
  type parallelizeFactory<T = number | object | any[]> = parallelizeCallback<T>;
  /**
   * @method parallelize()
   * @for kity.Utils
   ...
   * ```
   */
  function parallelize<T = number | object | any[]>(op: parallelizeCallback<T>): parallelizeFactory<T>;
